...
export default function LeaveCampaignForm({ campaign }: { campaign: Campaign }) {
  ...
  const { trigger: leaveCampaign, isMutating: isLoading } = useAction({
    ...
    onError: (error) => {
      showToast({ message: "Erro ao sair da campanha", variant: "error", title: "Erro" });
    },
  });
      ...
      <dl className="mt-6 space-y-6 divide-y divide-gray-100 border-t border-gray-200 text-sm leading-6">
        <div className="items-center pt-6 sm:flex">
          ...
          <dd className="mt-1 flex flex-col items-center justify-between gap-4 gap-x-6 sm:mt-0 sm:flex-auto md:flex-row md:gap-0">
            <Paragraph>
              Ao deixar a campanha, seu círculo de apoio será mantido, mas qualquer
              referência a você (nome, email e etc.) será removida.
            </Paragraph>
            <Button variant="danger" onClick={() => setShowCampaignLeaveModal(true)}>
              Sair da Campanha
            </Button>
          </dd>
        </div>
            ...
            <div className="mt-3 text-center sm:mt-5">
              ...
              <div className="mt-2">
                <p className="text-sm text-gray-500">
                  Você realmente deseja deixar a campanha? Você não poderá mais contribuir
                  com a campanha e não receberá mais atualizações.
                </p>
              </div>
            </div>
          </div>
