...
function ProfileUpdatedInput({ type, form }: { type: string; form: UseFormReturn }) {
  switch (type) {
    case "birthDate":
      return (
        <InputMask
    ...
    case "password":
      return (
        <div className="flex flex-col gap-4 lg:flex-row">
          <PasswordField hform={form} name="password" placeholder="Nova Senha" />
          <PasswordField
            hform={form}
...
function ProfileUpdateDescription({ type, form }: { type: string; form: UseFormReturn }) {
  switch (type) {
    case "birthDate":
      return (
    ...
    case "phone":
      return (
        <div className="text-gray-900">{formatPhone(form.getValues(type) || "")}</div>
      );
    case "password":
...
function ProfileUpdateButtons({
  ...
  setShowFields: React.Dispatch<
    SetStateAction<{ [key: string]: { name: string; show: boolean; label: string } }>
  >;
  trigger: any;
  form: UseFormReturn<any, any, any>;
}) {
