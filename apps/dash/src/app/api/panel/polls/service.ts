...
export async function readPollsStats(request: { campaignId: string }) {
  ...
  const pollsVotes = joinPollVotes(
    polls.flatMap((poll) => poll.PollAnswer)
  ).length;
  ...
  const commentsInPollsChange =
    Math.round(
      ((commentsInPolls - commentsInPollsLastWeek) / commentsInPollsLastWeek) *
        100
    ) || 0;
...
export async function answerPoll(
  ...
  const parsedAnswers = request.questions.map((question) => {
    ...
    if (
      ...
      const filteredAnswers = Object.keys(options).filter(
        (key) => options[key]
      );
